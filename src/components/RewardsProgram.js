...
const RewardsProgram = () => {
  ...
  const getRewards = transaction => {
    ...
    if (transaction.purchaseAmount > 100) {
      ...
      transaction.purchaseAmount <= 100 &&
      transaction.purchaseAmount > 50
    ) {
    ...
    for (let customer in groupedTransactions) {
      ...
      for (let transaction of groupedTransactions[customer]) {
        ...
        if (date in rewardsByMonthAndYear) {
          const updatedTotalRewardsByMonthAndYear =
            rewardsByMonthAndYear[date] + rewards;
  ...
  return (
    <div className="app">
      ...
      <button onClick={() => provideTransactions()}>Calculate Rewards</button>
